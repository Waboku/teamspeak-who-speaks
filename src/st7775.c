...
#if FLIPPED
static const uint16_t command_data[] = {0x011C, 0x0100,  0x1220, 0x0808, 0x0002, 0x1038, 0x1411, 0x0202, 0x0313, 0x0000, 0x00DB, 0x0000, 0x0000, 0x00DB, 0x0000, 0x00AF, 0x0000, 0x00DB, 0x0000, 0x0000, 0x0300, 0x0103, 0x2011, 0x0703, 0x0000, 0x0400, 0x0107, 0x2011, 0x0703, 0x0000, 0x0000, 0x1017};
#else
...
#if FLIPPED
	writeWr(HORI_ADDR_SET, LCD_X_MAX - position.x);
#else
...
void st7775_setRegion(screen_pos_t start, screen_pos_t end){
	...
	#if FLIPPED
		writeWr(HORI_ADDR_END_POS, LCD_X_MAX - start.x);
		writeWr(HORI_ADDR_START_POS, LCD_X_MAX - end.x);
	#else
